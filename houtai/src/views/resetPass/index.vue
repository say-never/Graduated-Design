...
<script>
	...
	import {putongyonghuUpdatePassword} from "@/api/putongyonghu/putongyonghuApi";	
				...
				this.$refs[editData].validate(valid => {
					if (valid) {
						if (this.editForm.newPassword === this.editForm.newPassword2) {
							...
							else if (this.userInfo.roles[0] == 'putongyonghu') {
								putongyonghuUpdatePassword(this.editForm).then(res => {
									if (res.code == "0") {
										this.$message.success('修改成功，为了安全，系统将自动清空session，请您重新登陆')
										setTimeout(() => {
											Session.clear();
